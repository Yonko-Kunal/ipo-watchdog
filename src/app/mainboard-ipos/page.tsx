	...
	const fetchIPOs = async () => {
		try {
			...
			if (response.data.success) {
				// Filter only Mainboard IPOs
				const mainboardIPOs = response.data.data.filter(
					(ipo: IPOItem) => ipo.type === "Mainboard"
				);
				setData(mainboardIPOs);
			} else {
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					<h1 className="text-3xl font-bold tracking-tight">
						Mainboard IPO Market
					</h1>
					<p className="text-muted-foreground mt-1 max-w-xl">
						Real-time tracking of Mainboard Initial Public Offerings. Stay
						updated with the latest GMP trends and listing gains.
					</p>
				</div>
				<Button
					variant="outline"
				...
				<Table>
					...
					<TableBody>
						{loading ? (
							...
							<TableRow>
								<TableCell colSpan={6} className="h-48 text-center">
									<p className="text-zinc-500">No Mainboard IPOs found.</p>
								</TableCell>
							</TableRow>
						) : (
							data.map((ipo, index) => {
